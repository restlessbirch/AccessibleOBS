...
$items = @(
  ...
  "SECURITY.md",
  "LICENSE",
  # Оба текста кладём в архив намеренно. Исходный код отдаётся под второй
  # версией с оговоркой «или новее», а собранная программа — под третьей,
  # потому что линкуется с библиотеками под Apache-2.0. Получатель должен
  # иметь на руках обе, иначе он не может воспользоваться ни тем, ни другим.
  "LICENSE-GPL-2.0.txt",
  "LICENSE-GPL-3.0.txt",
