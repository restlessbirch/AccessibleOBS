...
async fn session(
    ...
    if let Some(auth) = hello.pointer("/d/authentication") {
        if password.is_empty() {
            bail!(
                "OBS WebSocket требует пароль, а он не задан. Запустите START_FRIEND.bat — он создаст и сохранит пароль."
            );
        }
...
fn close_error(frame: Option<CloseFrame>) -> anyhow::Error {
    ...
    match u16::from(frame.code) {
        4009 => anyhow!(
            "OBS отклонил пароль WebSocket. Запустите START_FRIEND.bat — он пересоздаст пароль и настроит OBS."
        ),
...
mod tests {
    ...
    fn wrong_password_close_tells_the_user_what_to_run() {
        ...
        assert!(msg.contains("START_FRIEND.bat"), "названо решение: {msg}");
    }
